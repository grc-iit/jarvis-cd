...
class Daos(Application):
    ...
    def _DefineInit(self):
        ...
        if 'PREPARE_NVME' in self.config and self.config['PREPARE_NVME']:
            ...
            ExecNode(f"{os.path.join(self.config['DAOS_ROOT'], 'bin', 'daos_server')} -o {self.shared_dir}/daos_server.yaml storage prepare --nvme-only").Run()
            EchoNode("Select NVMe").Run()
            ExecNode(f"{os.path.join(self.config['DAOS_ROOT'], 'bin', 'daos_server')} -o {self.shared_dir}/daos_server.yaml storage scan")
