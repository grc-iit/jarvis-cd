...
class ResourceGraphManager:
    ...
    def _collect_from_nodes(self, nodes: List[str], benchmark: bool, duration: int):
        ...
        def collect_from_node(hostname: str) -> Dict[str, Any] | None:
            ...
            try:
                ...
                if (
                    ...
                    single_host_hostfile = Hostfile(hosts=[hostname])
    ...
    def _save(self):
        """Save the graph atomically through JARVIS's configured state boundary."""
    ...
    def filter_by_type(self, dev_type: str):
        ...
        for hostname, devices in filtered.items():
            ...
            for device in devices:
                perf_info = ""
                if (
                    device["4k_randwrite_bw"] != "unknown"
                    and device["1m_seqwrite_bw"] != "unknown"
                ):
                    perf_info = (
                        f" [4K: {device['4k_randwrite_bw']}, "
                        f"1M: {device['1m_seqwrite_bw']}]"
                    )
