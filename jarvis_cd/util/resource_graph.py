...
class ResourceGraph:
    ...
    def load_from_file(self, input_path: Path):
        ...
        if "fs" in data:
            ...
            raise ValueError(
                f"Invalid resource graph format in {input_path}. Expected 'fs' section."
            )
    ...
    def print_common_storage(self):
        ...
        for mount_point, devices in self.common_mounts.items():
            ...
            for device in devices:
                perf_info = ""
                if (
                    device.get("4k_randwrite_bw", "unknown") != "unknown"
                    and device.get("1m_seqwrite_bw", "unknown") != "unknown"
                ):
                    perf_info = f" [4K: {device.get('4k_randwrite_bw', 'unknown')}, 1M: {device.get('1m_seqwrite_bw', 'unknown')}]"

                logger.info(
                    f"  {device['hostname']}: {device['device']} ({device['avail']}, {device['dev_type']}){perf_info}"
                )
    ...
    def print_node_details(self, hostname: str):
        ...
        for device in devices:
            ...
            if device.get("4k_randwrite_bw", "unknown") != "unknown":
                logger.info(
                    f"  4K Random Write: {device.get('4k_randwrite_bw', 'unknown')}"
                )
            if device.get("1m_seqwrite_bw", "unknown") != "unknown":
                logger.info(
                    f"  1M Sequential Write: {device.get('1m_seqwrite_bw', 'unknown')}"
                )
