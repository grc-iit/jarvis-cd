...
class SCSRepoSetup(Package):
    def _LocalInstall(self):
        ...
        GitNode(**self.config['scs_repo'], method=GitOps.CLONE, collect_output=False, print_output=True).Run()
    ...
    def _LocalUpdate(self):
        ...
        GitNode(**self.config['scs_repo'], method=GitOps.UPDATE, collect_output=False, print_output=True).Run()
